    ...
    try {
      withEnv(["JAVA_HOME=${ tool "$jdk" }",
        ...
        configFileProvider(
          ...
          sh "mvn --no-transfer-progress -s $GLOBAL_MVN_SETTINGS -DsettingsPath=$GLOBAL_MVN_SETTINGS -Pci -V -B -e -Djetty.testtracker.log=true $cmdline -Dunix.socket.tmp=/tmp/unixsocket"
        }
      }
    }
    finally
